...
const addClass = (inicial, name) => (name === '' ? inicial : `${inicial} ${name}`);
const withDirections = animation => [`${animation} up`, `${animation} down`, `${animation} left`, `${animation} right`];
...
const buildAnimation = (name, direction) => `transition animating ${name} ${direction ? 'in' : 'out'} visible`;

const classBuilder = (visualState, animation) => {
  switch (visualState) {
    case opening:
